...
#Wrap tqdm() around the range() call in a for-loop to show a loading progress
#bar in the console.
from tqdm import tqdm
###############################################################################
# 1. Main Switchboard #########################################################
###############################################################################
#Choose an integer number of bodies to explore between 2 and 10.
N_ = 3
...
""" Below is a set of ICs simlar to our solar system.
...
#""" Here is a set of ICs not very similar to our solar system.
m0 = 1e30
m1 = 2e30
m2 = 3e30
m3 = 2.5e30
...
Outputs:
    ...
    [2] p = momentum data (3D numpy array)
    [3] KE = kinetic energy data (3D numpy array)
    [4] T = total kinetic energy data (3D numpy array)
    [5] U = potential energy data (3D numpy array)
    [6] t = time data (1D numpy array)
"""
def orbit(N, t0, tf, dt, m, r0, v0, method):
    ...
    Purpose:
        ...
        vectors for a number of gravitationally interacting bodies given each of
        their positions.
    Input:
    ...
    #def pot(r):
    #    #U=np.zeros([N,1])
    #    #Each body's self-potential energy has to to with its position relative
    #    #to each other body.
    #    for i in range(N):
    #        #j is a list of indices of all bodies other than the ith body.
    #        j = list(range(i))+list(range(i+1,N))
    #        #Note each body's self potential energy a sum of N-1 terms.
    #        for k in range(N-1):
    #            U=-G*m[i]*m[j[k]]/LA.norm(r[i]-r[j[k]])
    #            print(U)
    #        print('flag')
    #    return U
    ...
    def pot(r):
        ...
        j = list(range(0))+list(range(0+1,N))
        #Note each body's self potential energy a sum of N-1 terms.
        for k in range(N-1):
            U=-G*m[0]*m[j[k]]/LA.norm(r[0]-r[j[k]])+U
            #print(U)
        #print('flag')
        return U
    
    #U = np.zeros((steps,N,1))
    U = np.zeros((steps,1))
    #Derive potential energy data from the position data.
    for i in range(steps):
        U[i] = pot(r[i])
            
    #Lastly, derive kinetic energy and momentum data from the velocity data.
    Tt = np.zeros((steps,1)) #total kinetic energy of the system
    ...
    return r, v, p, KE, T, Tt, U, t

###############################################################################
# 5. Data Generation ##########################################################
###############################################################################
r, v, p , KE, T, Tt, U, t = orbit(N_, t0_, tf_, dt_, m_, r0_, v0_, method_)

###############################################################################
# 6. Figures ##################################################################
###############################################################################
#Select a body index from 0, 1, ..., N-1. Phase space and energy plots will be
#generated for the selected body.
mf = 1
...
ax5.plot(r[lo:hi, mf, 0], p[lo:hi, mf, 0], color=c_[mf], label=labs[mf], alpha=a_)
...
ax6.plot(r[lo:hi, mf, 1], p[lo:hi, mf, 1], color=c_[mf], label=labs[mf], alpha=a_)
...
ax7.plot(r[lo:hi, mf, 2], p[lo:hi, mf, 2], color=c_[mf], label=labs[mf], alpha=a_)
...
ax8.plot(r[lo:hi, mf, 0], KE[lo:hi, mf, 0], color=c_[mf], label=labs[mf], alpha=a_)
...
ax9.plot(r[lo:hi, mf, 1], KE[lo:hi, mf, 1], color=c_[mf], label=labs[mf], alpha=a_)
...
ax10.plot(r[lo:hi, mf, 2], KE[lo:hi, mf, 2], color=c_[mf], label=labs[mf], alpha=a_)
plt.savefig('fig10', bbox_inches='tight')

#------------------------------------------------------------------------------
#Total Kinetic Energy over Time for All Bodies
fig11 = plt.figure(11, facecolor='white')
ax11 = fig11.add_subplot(111)
plt.title(r'%s'%Nstr+r'$\mathrm{Orbiting\ Bodies\ }$'+'\n'
          +r'$\mathrm{Total\ Kinetic\ Energy\ vs.\ Time}$', y=1.05)
ax11.set_xlabel(r'$\mathrm{Time}\ \mathrm{(s)}$')
ax11.set_ylabel(r'$\mathrm{Total\ Kinetic\ Energy}\ \mathrm{(J)}$')
for i in range(N_):
...
#------------------------------------------------------------------------------
#Energy over Time for All Bodies
fig12 = plt.figure(12, facecolor='white')
ax12 = fig12.add_subplot(111)
plt.title(r'%s'%Nstr+r'$\mathrm{Orbiting\ Bodies\ }$'+'\n'
          +r'$\mathrm{Total\ Kinetic\ (T)\ and\ Potential\ (U)\ }$'
          +r'$\mathrm{Energy \ vs.\ Time}$', y=1.05)
ax12.set_xlabel(r'$\mathrm{Time}\ \mathrm{(s)}$')
ax12.set_ylabel(r'$\mathrm{Energy}\ \mathrm{(J)}$')
ax12.plot(t, Tt, color='black', label = r'$T_{\mathrm{total}}$', alpha=a_)
ax12.plot(t, U, color='red', label = r'$U_{\mathrm{total}}$', alpha=a_)
#for i in range(N_):
#    ax12.plot(t, U[:, i, :], color=c_[i], label=labs[i], alpha=a_)
