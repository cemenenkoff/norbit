...
def orbit(N, t0, tf, dt, m, r0, v0, method):
    ...
    def pot(r):
        ...
        for i in range(N):
            ...
            for k in range(N-1):
                U[i]=-G*m[i]*m[j[k]]/LA.norm(r[i]-r[j[k]])
        return U
    
    U = np.zeros((steps+1,N,1))
    print(np.size(U[1]), np.size(pot(r[1])))
    ...
    T = np.zeros((steps+1,N,1)) #array for total kinetic energy data
    KE = np.zeros((steps+1,N,3)) #array for 3D kinetic energy data
    p = np.zeros((steps+1,N,3)) #array for 3D momentum data
...
# 5. Data Generation ##########################################################
###############################################################################
r, v, p , KE, T, t = orbit(N_, t0_, tf_, dt_, m_, r0_, v0_, method_)
