...
method_ = 'vv' #Choose between 'ec' and 'vv'.
###############################################################################
G = 6.67e-11 #universal gravitational constant in units of m^3/kg*s^2
end_time = 10*365.26*24*3600 # Define the end time in seconds (e.g. 7 years).
...
m0 = 2.0e30 #mass of the sun in kg
m1 = 3.285e23 #mass of Mercury in kg
m2 = 4.8e24 #mass of Venus in kg
m3 = 6.0e24 #mass of Earth in kg
...
###############################################################################
#There are three main functions here, two(), three(), and four(), each of which
#calculate orbital trajectories for two, three, and four bodies respectively.
#Look to two() for in-depth comments on how the logic is constructed. three()
#and four() show how this method can be generalized. This lays a foundation for
#a general N-body function, but that will be saved for later if at all.
def two(m, method):
    ...
    if method == 'vv':
        #Below (but commented out) is the first attempt at the velocity verlet
        #method, given by this useful pdf:
        #http://young.physics.ucsc.edu/115/leapfrog.pdf
        #for i in range(steps):
        #    v_iphalf = v[i] + (1/2)*dt*accel(r[i])
        #    r[i+1] = r[i] + dt*v_iphalf
        #    v[i+1] = v_iphalf + (1/2)*dt*accel(r[i+1])
        
        #Instead though, Wikipedia shows us that we can eliminate the half-way
        #velocity calculation IF the acceleration function depends on position
        #only. Since we take our frame of reference to be stationary at the
        #origin, there won't be any centrifugal or Coriolis forces, so accel()
        #only depends on the distances between the bodies since their masses
        #do not change, hence we are justified in using this slightly simpler
        #implementation. See: https://en.wikipedia.org/wiki/Verlet_integration
        for i in range(steps):
            r[i+1] = r[i] + dt*v[i] + (1/2)*dt*accel(r[i])
            v[i+1] = v[i] + (1/2)*dt*(accel(r[i]) + accel(r[i+1]))
...
def three(m, method):
    ...
    def accel(r):
        a = np.zeros([3,3])
        #The acceleration of m0 has to do with forces from m1 and m2, etc.
        a[0] = G*(m[1]/LA.norm(r[0]-r[1])**3*(r[1]-r[0])
...
def four(m, method):
    ...
    v[0] = np.array([[0.0, 0.0, 0.0],
                     ...
                     [-4.0, -0.5, -3.0],
                     [7.0, 0.5, 2.0]])*1e3
    #The acceleration of m0 has to do with forces from m1, m2, and m3, etc.
    def accel(r):
        a = np.zeros([4,3])
        a[0] = G*(m[1]/LA.norm(r[0]-r[1])**3*(r[1]-r[0])
                + m[2]/LA.norm(r[0]-r[2])**3*(r[2]-r[0])
...
#Decide the scaling of the 3D plot. If zoom_order = 0, default scaling is used.
zoom_order = 0#1e13
if zoom_order != 0:
    ax1.set_xlim(-0.1*zoom_order,0.1*zoom_order)
    ax1.set_ylim(-0.1*zoom_order,0.1*zoom_order)
    ax1.set_zlim(-0.1*zoom_order,0.1*zoom_order)
