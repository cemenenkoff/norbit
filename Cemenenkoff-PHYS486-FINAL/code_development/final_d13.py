...
"""#Another set of ICs (IN PROGRESS)
m_ = [m0, m1, m1, m1, m1, m1, m1, m1, m1, m1]
c_ = [c0, c1, c1, c1, c1, c1, c1, c1, c1, c1]
r0_ = np.array([[  0.0,  0.0,  0.0 ], #0
                [  6.0, -5.0,  4.0 ], #1
                [ -6.0,  5.0, -4.0 ], #2
                [ -3.0,  2.5, -2.0 ], #3
                [ -1.0,  0.0,  0.0 ], #4
...
def orbit(N, t0, tf, dt, m, r0, v0, method):
    ...
    for i in range(steps):
        for j in range(N):
            T[i,j,0] = (m[j]/2)*sum(v2[i,j,:]) #KE=(m/2)*(vx^2+vy^2+vz^2)
            p[i,j,0] = m[j]*v[i,j,0] #px=m*vx
            p[i,j,1] = m[j]*v[i,j,1] #py=m*vy
            p[i,j,2] = m[j]*v[i,j,2] #pz=m*vz
...
# 5. Data Generation ##########################################################
###############################################################################
r, v, p , T, t = orbit(N_, t0_, tf_, dt_, m_, r0_, v0_, method_)
...
#------------------------------------------------------------------------------
fig5 = plt.figure(5, facecolor='white') #x-component phase space for m1
ax5 = fig5.add_subplot(111)
plt.title(r'%s'%Nstr+r'$\mathrm{Orbiting\ Bodies\ }$'
          +r'$\mathrm{x-Component\ Phase\ Space\ for\ }m_1$', y=1.05)
ax5.set_xlabel(r'$\mathrm{x-position}\ \mathrm{(m)}$')
ax5.set_ylabel(r'$\mathrm{x-momentum}\ \mathrm{(kg\cdot\frac{m}{s})}$')
ax5.plot(r[:, 1, 0], p[:, 1, 0], color=c_[1], label=labs[1], alpha=a_)
ax5.legend(loc='lower right')
#------------------------------------------------------------------------------
fig6 = plt.figure(6, facecolor='white') #y-component phase space for m1
ax6 = fig6.add_subplot(111)
plt.title(r'%s'%Nstr+r'$\mathrm{Orbiting\ Bodies\ }$'
          +r'$\mathrm{y-Component\ Phase\ Space\ for\ }m_1$', y=1.05)
ax6.set_xlabel(r'$\mathrm{y-position}\ \mathrm{(m)}$')
ax6.set_ylabel(r'$\mathrm{y-momentum}\ \mathrm{(kg\cdot\frac{m}{s})}$')
ax6.plot(r[:, 1, 1], p[:, 1, 1], color=c_[1], label=labs[1], alpha=a_)
ax6.legend(loc='lower right')
#------------------------------------------------------------------------------
fig7 = plt.figure(7, facecolor='white') #z-component phase space for m1
ax7 = fig7.add_subplot(111)
plt.title(r'%s'%Nstr+r'$\mathrm{Orbiting\ Bodies\ }$'
          +r'$\mathrm{z-Component\ Phase\ Space\ for\ }m_1$', y=1.05)
ax7.set_xlabel(r'$\mathrm{z-position}\ \mathrm{(m)}$')
ax7.set_ylabel(r'$\mathrm{z-momentum}\ \mathrm{(kg\cdot\frac{m}{s})}$')
ax7.plot(r[:, 1, 2], p[:, 1, 2], color=c_[1], label=labs[1], alpha=a_)
ax7.legend(loc='lower right')


#------------------------------------------------------------------------------
fig8 = plt.figure(8, facecolor='white') #kinetic energy over time
ax8 = fig8.add_subplot(111)
plt.title(r'%s'%Nstr+r'$\mathrm{Orbiting\ Bodies\ }$'
          +r'$\mathrm{Kinetic\ Energy\ vs.\ Time}$', y=1.05)
ax8.set_xlabel(r'$\mathrm{Time}\ \mathrm{(s)}$')
ax8.set_ylabel(r'$\mathrm{Kinetic\ Energy}\ \mathrm{(J)}$')
#ax8.plot(t, KE[:, 1, :], color=c_[1], label=labs[1], alpha=a_)
for i in range(N_):
    ax8.plot(t, T[:, i, :], color=c_[i], label=labs[i], alpha=a_)
