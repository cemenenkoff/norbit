...
# 1. Main Switchboard #########################################################
###############################################################################
#Choose an integer number of bodies to explore. Make sure the initial
#conditions data set is large enough for this choice.
N_ = 2
...
gen_fig2 = True #fig1 as viewed from the +x-axis
gen_fig3 = True #fig1 as viewed from the +y-axis
gen_fig4 = True #fig1 as viewed from the +z-axis
gen_fig5 = True #x-Component Momentum Phase Space for mf
gen_fig6 = True #y-Component Momentum Phase Space for mf
gen_fig7 = True #z-Component Momentum Phase Space for mf
gen_fig8 = True #x-Component Kinetic Energy Phase Space for mf
gen_fig9 = True #y-Component Kinetic Energy Phase Space for mf
gen_fig10 = True #z-Component Kinetic Energy Phase Space for mf
gen_fig11 = True #Total Kinetic Energy over Time for Each Body
...
#IC1: max(N)=10
#This set of ICs is similar to our own solar system, but the resulting orbital
#trajectories are not co-planar. Place/remove a hastag (#) in front of the
#triple double quote (""") immediately below to enable/disable IC1.
"""
...
#trajectories are far less stable than those resulting from IC1. Place/remove a
#hastag (#) in front of the triple double quote (""") immediately below to
#enable/disable IC2.
#""" 
m0 = 1e30
m1 = 2e30
...
Outputs:
    ...
    [5] Tt = total kinetic energy data for the system (2D numpy array)
    [6] Ut = total potential energy data for the system (2D numpy array)
...
def orbit(N, t0, tf, dt, m, r0, v0, method):
    ...
    Input:
        [0] r = 3D position vectors for all bodies at a certain time step
                (Nx3 numpy array)
    Output:
        [0] U = total gravitational potential energy (float)
    """
    def PE(r):
        U = 0
        for i in range(N):
    ...
    #Derive total system potential energy data from the position data.
    Ut = np.zeros((steps,1))
    ...
    return r, v, p, KE, T, Tt, Ut, t

###############################################################################
# 5. Data Generation ##########################################################
###############################################################################
r, v, p , KE, T, Tt, Ut, t = orbit(N_, t0_, tf_, dt_, m_, r0_, v0_, method_)
...
plt.title(r'%s'%Nstr+r'$\mathrm{Orbiting\ Bodies\ }$'+'\n'
          +r'$\mathrm{Total\ Kinetic\ and\ Potential\ Energies \ vs.\ Time}$',
          y=1.05)
...
ax12.plot(t, Ut, color='red', label = r'$U_{\mathrm{total}}$', alpha=a_)
