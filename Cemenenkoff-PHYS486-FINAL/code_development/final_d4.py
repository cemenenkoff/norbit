...
###############################################################################
N = 2 #Choose a number of bodies to explore between 2 and 5.
...
end_time = 30*365.26*24*3600 # Define the end time in seconds (e.g. 20 years).
...
def two(m, method):
    ...
    if method == 'fr':
        #Here is a Forest-Ruth implementation. See:
        #http://young.physics.ucsc.edu/115/leapfrog.pdf
        th = 1/(2-2**(1/3)) #Define a multiplicative constant.
        for i in range(steps):
            r[i] = r[i] + th*(dt/2)*v[i]
            v[i] = v[i] + th*dt*accel(r[i])
            r[i] = r[i] + (1-th)*(dt/2)*v[i]
            v[i] = v[i] + (1-2*th)*dt*accel(r[i])
            r[i] = r[i] + (1-th)*(dt/2)*v[i]
            v[i+1] = v[i] + th*dt*accel(r[i])
            r[i+1] = r[i] + th*(dt/2)*v[i]
...
def three(m, method):
    ...
    if method == 'fr':
        ...
        for i in range(steps):
            r[i+1] = r[i] + th*(dt/2)*v[i]
            v[i+1] = v[i] + th*dt*accel(r[i])
            r[i+1] = r[i] + (1-th)*(dt/2)*v[i]
            v[i+1] = v[i] + (1-2*th)*dt*accel(r[i])
            r[i+1] = r[i] + (1-th)*(dt/2)*v[i]
            v[i+1] = v[i] + th*dt*accel(r[i])
            r[i+1] = r[i] + th*(dt/2)*v[i]
    return r, v
...
#five() extends the problem to four gravitationally interacting bodies.
def five(m, method):
...
ax2.plot(r[:, 0, 0], r[:, 0, 2], color=c[0], label=r'$m_0$', alpha=a_)
#For all times, plot (x,z) tuples for m1, etc.
ax2.plot(r[:, 1, 0], r[:, 1, 2], color=c[1], label=r'$m_1$', alpha=a_)
if N > 2:
    ax2.plot(r[:, 2, 0], r[:, 2, 2], color=c[2], label=r'$m_2$', alpha=a_)
if N > 3:
    ax2.plot(r[:, 3, 0], r[:, 3, 2], color=c[3], label=r'$m_3$', alpha=a_)
if N > 4:
    ax2.plot(r[:, 4, 0], r[:, 4, 2], color=c[4], label=r'$m_4$', alpha=a_)
ax2.axis('equal')
