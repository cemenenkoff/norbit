...
def orbit(N, t0, tf, dt, method, m, r0, v0):
    ...
    def accel(r):
        ...
        for i in range(N):
            ...
            #Note each body's acceleration vector is a sum of terms, so for
    ...
    #print(np.shape(r[i]), np.shape(v[i]), np.shape(accel(r[i])))
...
# 6. Figures ##################################################################
###############################################################################
#Generate an ascending list of integers and then change the elements into
#strings for use in figure titles.
#words = list(str(range(N_)))
